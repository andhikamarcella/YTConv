...
A third-party conversion website receives the URL, IP address, browser metadata, and potentially account data. Some sites also use intrusive ads, tracking, fake download buttons, or unsafe executables. YTConv 1.7.7 therefore keeps conversion local and publishes GitHub Pages only as a tutorial—never as a server-side ripper.
...
`--upscale 4k` performs deterministic FFmpeg Lanczos scaling. It creates a real 3840×2160 file but does not claim AI super-resolution or recovered detail. A portable, tested AI model is not bundled in 1.7.7 because that would be misleading on unsupported devices.
