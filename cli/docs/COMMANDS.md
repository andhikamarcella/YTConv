# YTConv 1.7.7 Command Reference
...
donate                       Choose Ko-fi (global) or Saweria (Indonesia)
```
...
The browser handoff is optional, restricted to the two documented HTTPS pages, and returns an interactive session to the YTConv home screen after five seconds. When a browser cannot be opened, YTConv copies or prints the link. See [Donate](DONATE.md).
...
YTConv 1.7.7 verifies that the final path exists and matches the requested mode:
...
YTConv 1.7.7 keeps subtitles disabled by default unless explicitly requested. Missing subtitles should not fail the main media download.
...
YTConv 1.7.7 defaults to `mark`, which adds chapters without cutting media. `remove` cuts matching segments and must be requested explicitly.
...
If an archived URL has no real output file, YTConv 1.7.7 retries that item once without the archive. A second zero-file result fails instead of reporting success.
