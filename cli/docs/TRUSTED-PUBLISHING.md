...
YTConv 1.7.7 uses GitHub Actions OpenID Connect instead of a long-lived npm publication token.
...
Publication is manual and requires confirmation. Before publishing, the workflow checks the exact package and CLI versions and runs the full prepack suite. After publishing, it verifies `ytconv@1.7.7`, checks the `latest` dist-tag, compares the registry tarball SHA-256, and creates the corresponding GitHub Release.
