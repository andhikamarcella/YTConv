...
test('documentation URLs use the canonical 1.7.7 release branch', () => {
  assert.match(documentationUrl('faq'), /andhikamarcella\/YTConv\/blob\/release\/ytconv-1\.7\.7\/cli\/docs\/FAQ\.md$/u);
  assert.match(documentationUrl('index'), /andhikamarcella\/YTConv\/tree\/release\/ytconv-1\.7\.7\/cli\/docs$/u);
  assert.match(documentationUrl('donate'), /cli\/docs\/DONATE\.md$/u);
});

test('help-center panels are responsive and discoverable', () => {
  assert.match(aboutText(50), /YTConv 1\.7\.7/u);
